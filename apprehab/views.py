...
def assign_patient(request, staff_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('staff_list')  # Redirect after assigning patients
    ...
    if request.method == 'POST':
        ...
        if staff_id:
            try:
                ...
                application.save()
                success_message = f'Successfully assigned {staff.name} to this application.'
                ...
                error_message = 'Selected staff member not found.'
            ...
            application.save()
            success_message = 'Staff assignment removed.'
...
def review_application(request, application_id):
    ...
    if action == 'approve':
        ...
        if start_date:
            ...
            application.status = 'approved'
            application.save()
            messages.success(request, 'Application approved successfully. Please set a start date for the program.')
    elif action == 'reject':
...
def set_program_start_date(request, application_id):
    ...
    if request.method == 'POST':
        ...
        if start_date:
            application.start_date = start_date
            application.save()
            messages.success(request, 'Program start date set successfully.')
            return redirect('addiction_list')
...
def register(request):
    if request.method == 'POST':
        ...
        addiction_detail = request.POST['addiction_detail']
        ...
        if password1 == password2:
            ...
            try:
                send_registration_confirmation(email, username)
            except Exception as e:
...
def approve_application(request, application_id):
    ...
    if request.method == 'POST':
        ...
        if not start_date:
            ...
            return redirect('approve_application', application_id=application_id)
        ...
        from .email_utils import send_application_status_update
        try:
...
def generate_application_pdf(request, application_id):
    ...
    sanitized_name = ''.join(e for e in application.name if e.isalnum() or e == ' ').replace(' ', '_')
...
def submit_addiction(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            from .email_utils import send_application_confirmation
            try:
                ...
                print(f"Error sending application confirmation email: {e}")
